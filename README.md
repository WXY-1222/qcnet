...
### INTERACTION (DIGIR-format) Baseline

This repo now supports training/evaluation on DIGIR-preprocessed INTERACTION `.pkl` files
(e.g. `interaction_digir_all_12loc_h8_f12.pkl`).

1) Train on 8xA100 (Linux):
```bash
bash scripts/train_qcnet_interaction_8xa100.sh /data/sdb/bitwxy/interaction_data/interaction_digir_all_12loc_h8_f12.pkl
```
...
python val.py \
  ...
  --interaction_data_path /data/sdb/bitwxy/interaction_data/interaction_digir_all_12loc_h8_f12.pkl \
  --split val \
  --batch_size 4 \
  --devices 1 \
  --accelerator gpu
