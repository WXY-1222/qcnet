...
# Topology-aware teacher reranking for the full-replacement TopoSSM decoder.
# Seed49B's proposal/refine trajectory generator is loaded and frozen; only
# decoder.to_topo_aux_pi learns to rerank modes using corridor cues plus
# SafetyFT/QCNet teacher score/rank supervision.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_topo_teacher_rerank_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_topo_teacher_rerank_h8_f12_s5_k6_4gpu_20260505.log}"
...
if pgrep -af "train_qcnet.py|torchrun" | grep -q "bitwxy/qcnet"; then
  log "Another bitwxy QCNet training process is active; refusing to launch a concurrent run."
  pgrep -af "train_qcnet.py|torchrun" | grep "bitwxy/qcnet" || true
  exit 3
fi

rm -rf "${SAVE_ROOT}/lightning_logs/version_0"

log "Topo teacher-rerank supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_mamba_d_conv 4 \
  --topo_mamba_expand 2 \
  --topo_corridor_loss_weight 0.0 \
  --topo_score_loss_weight 0.0 \
  --topo_score_temperature 0.18 \
  --topo_aux_score \
  --topo_aux_score_only \
  --topo_aux_score_loss_weight 0.03 \
  --topo_aux_gt_score_loss_weight 0.20 \
  --topo_aux_score_mix 0.12 \
  --distill_propose_weight 0.0 \
  --distill_refine_weight 0.0 \
  --distill_score_weight 0.08 \
  --distill_rank_weight 0.20 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.056; then
      log "Teacher-rerank did not preserve ADE by epoch ${latest_epoch}: best_ade=${best_ade} > 0.056; stopping."
      kill_run
      exit 2
    fi
  fi
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 8 )); then
    if ! float_le "${best_ade}" "${BASELINE_ADE}"; then
      log "Teacher-rerank has not beaten Seed49B by epoch ${latest_epoch}: best_ade=${best_ade} > ${BASELINE_ADE}; stopping."
      kill_run
      exit 2
    fi
  fi
