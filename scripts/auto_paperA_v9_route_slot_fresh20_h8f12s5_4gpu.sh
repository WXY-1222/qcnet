...
SEED="${SEED:-17}"
RUN_TAG="${RUN_TAG:-20260507_paperA_v9_route_slot_fresh20_seed17}"
...
TOPO_GOAL_ANCHOR_BLEND="${TOPO_GOAL_ANCHOR_BLEND:-0.80}"
TOPO_POLYLINE_CONTROL_SCALE="${TOPO_POLYLINE_CONTROL_SCALE:-0.06}"
TOPO_ROUTE_SLOT_LONGITUDINAL_SCALE="${TOPO_ROUTE_SLOT_LONGITUDINAL_SCALE:-0.20}"
TOPO_ROUTE_SLOT_LATERAL_SCALE="${TOPO_ROUTE_SLOT_LATERAL_SCALE:-0.10}"
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_v9_route_slot_fresh20_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_v9_route_slot_fresh20_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA-v9 route-slot fresh 20-epoch run started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=proposal=route_slot_polyline endpoint_scale=${TOPO_MODE_ENDPOINT_SCALE} slot_long=${TOPO_ROUTE_SLOT_LONGITUDINAL_SCALE} slot_lat=${TOPO_ROUTE_SLOT_LATERAL_SCALE} slot_topk=${TOPO_ROUTE_SLOT_TOPK} lr=${LR}"
...
setsid "${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type route_slot_polyline \
  --topo_ssm_layers 2 \
