...
# Seed-stability probe for the current best full-replacement TopoSSM decoder
# setting: mode-specific endpoint proposal with topo_mode_endpoint_scale=0.20.
# Each seed starts from the same Seed49B base checkpoint and uses a unique
# save_root so this script never overwrites previous best runs.
...
RUN_TAG="${RUN_TAG:-20260505_scale200_seed_stability}"
...
run_seed() {
  ...
  local pid="$!"
  log "scale=${SCALE} seed=${seed} train PID ${pid}"
  ...
  while kill -0 "${pid}" 2>/dev/null; do
    ...
    log "scale=${SCALE} seed=${seed} latest_epoch=${latest_epoch:-na} latest_ade=${latest_ade:-na} best_ade=${best_ade:-na} best_path=${best_path:-na}"
    ...
    if [[ -n "${latest_epoch}" && -n "${latest_ade}" ]] && (( latest_epoch >= 1 )); then
      if ! float_le "${latest_ade}" "${STOP_LATEST_ADE}"; then
        log "scale=${SCALE} seed=${seed} unstable latest_ade=${latest_ade} > ${STOP_LATEST_ADE}; switching seed."
  ...
  best_path="$(json_value "${metrics}" best_path)"
  update_best_overall "${seed}" "${best_ade}" "${best_path}"
  log "scale=${SCALE} seed=${seed} finished. metrics=${metrics}"
  return 0
}
...
log "TopoSSM mode_endpoint scale=${SCALE} seed-stability sweep started"
...
log "Seed-stability sweep finished."
...
if [[ -n "${BEST_OVERALL_ADE}" ]] && float_le "${BEST_OVERALL_ADE}" "${BASELINE_ADE}"; then
  log "At least one seed reproduced or beat baseline ${BASELINE_ADE}."
  exit 0
fi
exit 1
