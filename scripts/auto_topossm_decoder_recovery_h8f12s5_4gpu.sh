...
# Recovery continuation after the teacher-rank probe regressed.
# Start from the best Seed49B checkpoint, keep goal proposals free, and use only
# light teacher refine/score regularization so ADE can recover before topology is
# pushed harder again.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_recovery_seed49warm_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_recovery_seed49warm_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "TopoSSM recovery supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_mamba_d_conv 4 \
  --topo_mamba_expand 2 \
  --topo_corridor_loss_weight 0.005 \
  --topo_score_loss_weight 0.015 \
  --topo_score_temperature 0.20 \
  --distill_propose_weight 0.0 \
  --distill_refine_weight 0.005 \
  --distill_score_weight 0.015 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.0555; then
      log "Behind recovery threshold at epoch ${latest_epoch}: best_ade=${best_ade} > 0.0555; stopping."
      kill_run
      exit 2
    fi
  fi
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 8 )); then
    if ! float_le "${best_ade}" 0.0545; then
      log "Still behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.0545; stopping."
      kill_run
      exit 2
    fi
  fi
