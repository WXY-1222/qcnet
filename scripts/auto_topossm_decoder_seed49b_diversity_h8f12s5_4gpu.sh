...
# Conservative follow-up from Seed49B:
# keep the proven goal_mlp TopoSSM decoder path, freeze the QCNet encoder, and
# add only a tiny mode-diversity loss so the six candidates cover more outcomes.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_seed49b_diversity_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_seed49b_diversity_h8_f12_s5_k6_4gpu_20260505.log}"
...
log "Seed49B diversity TopoSSM supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_score_loss_weight 0.01 \
  --topo_score_temperature 0.20 \
  --topo_diversity_loss_weight 0.001 \
  --topo_diversity_margin 0.15 \
  --distill_propose_weight 0.005 \
  --distill_refine_weight 0.02 \
  --distill_score_weight 0.02 \
...
while is_pid_running "${pid}"; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 3 )); then
    if ! float_le "${best_ade}" 0.058; then
      log "Diversity run behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.058; stopping."
      kill_run
      exit 2
    fi
  fi
