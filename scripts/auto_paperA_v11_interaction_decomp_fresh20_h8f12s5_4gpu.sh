...
SEED="${SEED:-17}"
RUN_TAG="${RUN_TAG:-20260507_paperA_v11_interaction_decomp_fresh20_seed17}"
...
STOP_IF_BEST_GT_E4="${STOP_IF_BEST_GT_E4:-0.120}"
STOP_IF_BEST_GT_E8="${STOP_IF_BEST_GT_E8:-0.090}"

SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_v11_interaction_decomp_fresh20_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_v11_interaction_decomp_fresh20_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA-v11 interaction-decomp fresh 20-epoch run started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=proposal=interaction_decomp_endpoint endpoint_scale=${TOPO_MODE_ENDPOINT_SCALE} lr=${LR}"
...
setsid "${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type interaction_decomp_endpoint \
  --topo_ssm_layers 2 \
