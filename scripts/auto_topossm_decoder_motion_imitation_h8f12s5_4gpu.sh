...
# Joint proposal + refinement imitation for the full-replacement TopoSSM decoder.
# Proposal-only imitation destabilized the frozen refiner, so this run trains the
# proposal path and temporal refinement path together while keeping the encoder
# and scoring head fixed.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_motion_imitation_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_motion_imitation_h8_f12_s5_k6_4gpu_20260505.log}"
...
log "Topo motion-imitation supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --distill_propose_weight 0.06 \
  --distill_refine_weight 0.18 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.057; then
      log "Motion imitation destabilized ADE by epoch ${latest_epoch}: best_ade=${best_ade} > 0.057; stopping."
      kill_run
      exit 2
    fi
  fi
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 6 )); then
    if ! float_le "${best_ade}" "${BASELINE_ADE}"; then
      log "Motion imitation has not beaten Seed49B by epoch ${latest_epoch}: best_ade=${best_ade} > ${BASELINE_ADE}; stopping."
      kill_run
      exit 2
    fi
  fi
