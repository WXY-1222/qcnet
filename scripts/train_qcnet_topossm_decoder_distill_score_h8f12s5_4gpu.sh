...
# Score-focused distillation:
# continue from the best teacher-distilled TopoSSM decoder checkpoint, reduce
# trajectory imitation, and keep stronger teacher guidance on mode ranking.

DATA_ROOT="${DATA_ROOT:-/data/sdb/bitwxy/interaction_data}"
SAVE_ROOT="${SAVE_ROOT:-/data/sdb/bitwxy/qcnet_data/qcnet_topossm_decoder_distill_score_h8_f12_s5_k6_4gpu_20260503}"
DATA_FILE="${DATA_FILE:-interaction_digir_all_12loc_h8_f12_s5.pkl}"
DATA_PKL="${DATA_ROOT}/${DATA_FILE}"
INIT_CKPT="${INIT_CKPT:-/data/sdb/bitwxy/qcnet_data/qcnet_topossm_decoder_distill_h8_f12_s5_k6_4gpu_20260503/lightning_logs/version_0/checkpoints/epoch=5-step=1290.ckpt}"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --distill_propose_weight "${DISTILL_PROPOSE_WEIGHT:-0.03}" \
  --distill_refine_weight "${DISTILL_REFINE_WEIGHT:-0.08}" \
  --distill_score_weight "${DISTILL_SCORE_WEIGHT:-0.15}" \
  --distill_temperature "${DISTILL_TEMPERATURE:-1.5}" \
  --distill_warmup_epochs "${DISTILL_WARMUP_EPOCHS:-2}"
