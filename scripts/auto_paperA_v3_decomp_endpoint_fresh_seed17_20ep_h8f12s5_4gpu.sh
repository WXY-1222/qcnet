...
# Fresh-from-scratch PaperA-v3 probe:
# decomposed endpoint proposal, but keep the original PaperA anchor-residual path.
...
SEED="${SEED:-17}"
RUN_TAG="${RUN_TAG:-20260506_paperA_v3_decomp_endpoint_seed17_20ep}"
...
TARGET_ADE="${TARGET_ADE:-0.05}"

SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_v3_decomp_endpoint_fresh_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_v3_decomp_endpoint_fresh_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA-v3 fresh 20-epoch probe started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=from_scratch decoder=topossm proposal=decomp_endpoint endpoint_scale=${TOPO_MODE_ENDPOINT_SCALE} epochs=${MAX_EPOCHS} lr=${LR}"
