...
# Right-side refinement sweep around the current best TopoSSM mode-endpoint scale.
# Current confirmed best before this script:
#   scale=0.12, seed=42, val_minADE=0.04884454
# This script preserves all previous runs and starts each scale from the same
# Seed49B base checkpoint, because continuing the best checkpoint has been less stable.
...
SCALES="${SCALES:-0.13 0.14 0.16}"
SEED="${SEED:-42}"
TARGET_ADE="${TARGET_ADE:-0.0480}"
BASELINE_ADE="${BASELINE_ADE:-0.04884454}"
SWITCH_IF_AFTER_EPOCH="${SWITCH_IF_AFTER_EPOCH:-3}"
SWITCH_ADE="${SWITCH_ADE:-0.0492}"
STOP_LATEST_ADE="${STOP_LATEST_ADE:-0.0575}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-120}"
RUN_TAG="${RUN_TAG:-20260505_refine}"
...
scale_tag() {
  "${PYTHON_BIN}" - "$1" <<'PY'
import sys
print(str(int(round(float(sys.argv[1]) * 1000))).zfill(3))
PY
}
...
run_scale() {
  ...
  local tag
  tag="$(scale_tag "${scale}")"
  local save_root="${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_scale${tag}_seed${SEED}_h8_f12_s5_k6_4gpu_${RUN_TAG}"
  local run_log="${LOG_DIR}/qcnet_topossm_decoder_mode_endpoint_scale${tag}_seed${SEED}_h8_f12_s5_k6_4gpu_${RUN_TAG}.log"
  ...
  log "===== START mode_endpoint refine scale=${scale} seed=${SEED} ====="
  ...
  while kill -0 "${pid}" 2>/dev/null; do
    ...
    log "scale=${scale} latest_epoch=${latest_epoch:-na} latest_ade=${latest_ade:-na} best_ade=${best_ade:-na} best_path=${best_path:-na}"
    update_best_overall "${scale}" "${best_ade}" "${best_path}"

    if [[ -n "${best_ade}" ]] && float_le "${best_ade}" "${TARGET_ADE}"; then
      log "scale=${scale} reached target best_ade=${best_ade} <= ${TARGET_ADE}; stopping sweep."
      stop_train "${save_root}"
      return 0
    fi
    if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= SWITCH_IF_AFTER_EPOCH )); then
      if ! float_le "${best_ade}" "${SWITCH_ADE}"; then
        log "scale=${scale} not promising after epoch ${latest_epoch}: best_ade=${best_ade} > ${SWITCH_ADE}; switching scale."
        stop_train "${save_root}"
        return 2
      fi
    fi
    if [[ -n "${latest_epoch}" && -n "${latest_ade}" ]] && (( latest_epoch >= 1 )); then
      if ! float_le "${latest_ade}" "${STOP_LATEST_ADE}"; then
        log "scale=${scale} unstable latest_ade=${latest_ade} > ${STOP_LATEST_ADE}; switching scale."
        stop_train "${save_root}"
        return 2
      fi
    fi
  ...
  best_path="$(json_value "${metrics}" best_path)"
  update_best_overall "${scale}" "${best_ade}" "${best_path}"
  log "scale=${scale} finished. metrics=${metrics}"
  if [[ -n "${best_ade}" ]] && float_le "${best_ade}" "${TARGET_ADE}"; then
    return 0
  fi
  return 2
...
log "TopoSSM mode_endpoint right-side scale refine sweep started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED} scales=${SCALES} target=${TARGET_ADE} baseline=${BASELINE_ADE}"

for scale in ${SCALES}; do
  status=0
  run_scale "${scale}" || status=$?
  if [[ "${status}" == "0" ]]; then
    log "Refine sweep reached target. best_scale=${BEST_OVERALL_SCALE} best_ade=${BEST_OVERALL_ADE} best_path=${BEST_OVERALL_PATH}"
    exit 0
  fi
done

log "Refine sweep finished without reaching target ${TARGET_ADE}."
...
if [[ -n "${BEST_OVERALL_ADE}" ]] && float_le "${BEST_OVERALL_ADE}" "${BASELINE_ADE}"; then
  log "Refine sweep found a checkpoint that beats baseline ${BASELINE_ADE}."
  exit 0
fi
exit 1
