...
# Real follow-up after the teacher-proposal oracle:
# train a full-replacement TopoSSM decoder with a hard corridor-query proposal
# head, without using teacher proposals at inference/training forward time.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_corridor_query_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_corridor_query_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "Corridor-query TopoSSM decoder run started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type corridor_query \
  --topo_goal_distance_weight 0.08 \
  --topo_goal_residual_scale 0.35 \
  --topo_goal_anchor_blend 0.8 \
  --topo_ssm_layers 2 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.060; then
      log "Corridor-query behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.060; stopping."
      kill_run
      exit 2
    fi
  fi
