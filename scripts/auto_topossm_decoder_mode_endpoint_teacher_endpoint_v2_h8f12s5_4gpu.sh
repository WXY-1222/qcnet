...
# Teacher-guided endpoint coverage probe for the full-replacement TopoSSM decoder.
# Starts from the current best mode_endpoint checkpoint and only adapts proposal-side
# modules, using SafetyFT teacher endpoints as a soft coverage target.
...
STOP_BAD_ADE="${STOP_BAD_ADE:-0.056}"
STOP_LATEST_ADE="${STOP_LATEST_ADE:-0.066}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-120}"

INIT_CKPT="${INIT_CKPT:-${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_h8_f12_s5_k6_4gpu_20260505/lightning_logs/version_0/checkpoints/epoch=4-step=1075.ckpt}"
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_teacher_endpoint_v2_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_mode_endpoint_teacher_endpoint_v2_h8_f12_s5_k6_4gpu_20260505.log}"
...
log "TopoSSM mode-endpoint teacher-endpoint-v2 supervisor started"
log "Repo=${REPO} GPUs=${GPU_IDS} target=${TARGET_ADE}"
log "Init=${INIT_CKPT}"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --distill_endpoint_weight 0.15 \
  --distill_refine_weight 0.0 \
  --distill_score_weight 0.0 \
  --distill_temperature 1.5 \
  --distill_warmup_epochs 0 \
  --topo_proposal_distill_only \
  > "${RUN_LOG}" 2>&1 &
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 2 )); then
    if ! float_le "${best_ade}" "${STOP_BAD_ADE}"; then
      log "Behind after epoch ${latest_epoch}: best_ade=${best_ade} > ${STOP_BAD_ADE}; stopping run."
      stop_train
      exit 2
    fi
  fi
