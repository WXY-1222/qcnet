...
# Diagnostic run: feed frozen SafetyFT/QCNet teacher proposals into the full
# TopoSSM decoder path. This isolates whether TopoSSM's own proposal head is
# the bottleneck before implementing a harder corridor-conditioned proposal.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_teacher_proposal_oracle_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_teacher_proposal_oracle_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "Teacher-proposal oracle TopoSSM decoder run started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_score_loss_weight 0.005 \
  --topo_score_temperature 0.20 \
  --distill_propose_weight 0 \
  --distill_refine_weight 0 \
  --distill_score_weight 0 \
  --distill_rank_weight 0 \
  --use_teacher_proposals \
  --teacher_proposal_source refine \
  --freeze_encoder \
  > "${RUN_LOG}" 2>&1 &
pid="$!"
log "PID ${pid}"
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 3 )); then
    if ! float_le "${best_ade}" 0.058; then
      log "Oracle still behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.058; stopping."
      kill_run
      exit 2
    fi
  fi
