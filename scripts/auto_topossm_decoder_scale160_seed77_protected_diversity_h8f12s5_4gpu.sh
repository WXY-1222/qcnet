...
# Short probe for the full-replacement TopoSSM decoder after the scale=0.16
# diagnosis: keep the current best mode protected and add only a light endpoint
# diversity margin to non-best modes.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_scale160_seed77_protected_diversity_h8_f12_s5_k6_4gpu_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_scale160_seed77_protected_diversity_h8_f12_s5_k6_4gpu_${RUN_TAG}.log}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-120}"

INIT_CKPT="${INIT_CKPT:-${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_scale160_seed77_h8_f12_s5_k6_4gpu_20260505_scale160_seed_stability/lightning_logs/version_0/checkpoints/epoch=1-step=430.ckpt}"
...
log "Protected diversity TopoSSM probe started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_mode_endpoint_scale 0.16 \
  --topo_endpoint_diversity_loss_weight 0.015 \
  --topo_endpoint_diversity_margin 0.16 \
  --topo_ssm_layers 2 \
  ...
  --distill_propose_weight 0.005 \
  --distill_refine_weight 0.01 \
  --distill_score_weight 0.01 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 2 )); then
    if ! float_le "${best_ade}" "0.0465"; then
      log "best_ade=${best_ade} remains > 0.0465 after epoch ${latest_epoch}; stopping as unpromising."
      stop_train
      break
    fi
  fi
