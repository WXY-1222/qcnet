...
# Scheme 2: harden TopoSSM proposals.
# This script does not replace the ABC or B-seed sweep scripts. It starts from
# the best light-distill seed49 checkpoint and switches proposal generation from
# a free goal MLP to corridor-goal queries conditioned on map/corridor features.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_hard_proposal_seed49init_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_hard_proposal_seed49init_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "TopoSSM hard-proposal supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_goal_distance_weight 0.10 \
  --topo_goal_residual_scale 0.20 \
  --topo_ssm_layers 2 \
  --topo_mamba_d_state 16 \
  --topo_mamba_d_conv 4 \
  --topo_mamba_expand 2 \
  --topo_corridor_loss_weight 0.04 \
  --topo_score_loss_weight 0.04 \
  --topo_score_temperature 0.16 \
  --distill_propose_weight 0.005 \
  --distill_refine_weight 0.02 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.056; then
      log "Hard proposal is behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.056; stopping."
      kill_run
      exit 2
    fi
  fi
