...
# Corridor-conditioned proposal supervisor.
# Keep the QCNet encoder and learn a corridor-aware residual goal proposal
# instead of snapping goals directly onto map nodes.
...
log "Corridor-residual supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type corridor_residual \
  --topo_goal_distance_weight 0.03 \
  --topo_goal_residual_scale 0.20 \
  --topo_goal_anchor_blend 1.0 \
  --topo_ssm_layers 2 \
