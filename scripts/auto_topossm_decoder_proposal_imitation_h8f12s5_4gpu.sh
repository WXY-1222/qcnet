...
# Teacher-guided proposal imitation for the full-replacement TopoSSM decoder.
# This is a clean diagnostic/train step after reranking plateaued: keep the
# Seed49B model mostly frozen and train only the TopoSSM proposal path to
# imitate SafetyFT/QCNet-quality proposal trajectories.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_proposal_imitation_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_proposal_imitation_h8_f12_s5_k6_4gpu_20260505.log}"
...
log "Topo proposal-imitation supervisor started"
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.057; then
      log "Proposal imitation destabilized ADE by epoch ${latest_epoch}: best_ade=${best_ade} > 0.057; stopping."
      kill_run
      exit 2
    fi
  fi
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 6 )); then
    if ! float_le "${best_ade}" "${BASELINE_ADE}"; then
      log "Proposal imitation has not beaten Seed49B by epoch ${latest_epoch}: best_ade=${best_ade} > ${BASELINE_ADE}; stopping."
      kill_run
      exit 2
    fi
  fi
