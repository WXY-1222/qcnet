...
# Clean fresh PaperA probe:
# add a warmup-only matched-mode endpoint auxiliary loss for the first 3 epochs
# while keeping the smaller Mamba LR / zero Mamba WD setting.
...
SEED="${SEED:-17}"
RUN_TAG="${RUN_TAG:-20260506_paperA_fresh_seed17_20ep_auxgoal_mambaopt}"
...
AUX_GOAL_LOSS_WEIGHT="${AUX_GOAL_LOSS_WEIGHT:-0.2}"
AUX_GOAL_LOSS_WEIGHT_DECAY="${AUX_GOAL_LOSS_WEIGHT_DECAY:-0.1}"
AUX_GOAL_WARMUP_EPOCHS="${AUX_GOAL_WARMUP_EPOCHS:-3}"
TRAIN_BATCH_SIZE="${TRAIN_BATCH_SIZE:-16}"
VAL_BATCH_SIZE="${VAL_BATCH_SIZE:-16}"
TEST_BATCH_SIZE="${TEST_BATCH_SIZE:-16}"
NUM_WORKERS="${NUM_WORKERS:-4}"
TARGET_ADE="${TARGET_ADE:-0.058}"
STOP_IF_BEST_GT_E4="${STOP_IF_BEST_GT_E4:-0.085}"

SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_fresh_auxgoal_mambaopt_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_fresh_auxgoal_mambaopt_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA fresh seed17 20-epoch aux-goal + Mamba-optimizer probe started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=from_scratch proposal=mode_endpoint scale=${TOPO_MODE_ENDPOINT_SCALE} lr=${LR} mamba_lr=${MAMBA_LR} wd=${WEIGHT_DECAY} mamba_wd=${MAMBA_WEIGHT_DECAY}"
log "aux_goal_schedule=${AUX_GOAL_LOSS_WEIGHT} -> ${AUX_GOAL_LOSS_WEIGHT_DECAY} decay with warmup_epochs=${AUX_GOAL_WARMUP_EPOCHS}"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --aux_goal_loss_weight "${AUX_GOAL_LOSS_WEIGHT}" \
  --aux_goal_loss_weight_decay "${AUX_GOAL_LOSS_WEIGHT_DECAY}" \
  --aux_goal_warmup_epochs "${AUX_GOAL_WARMUP_EPOCHS}" \
  --eval_batches 0 \
  --num_modes 6 \
  --eval_k 6 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if float_gt "${best_ade}" "${STOP_IF_BEST_GT_E4}"; then
