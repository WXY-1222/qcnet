...
# Scheme 2 safe retry: corridor-goal proposal with strong distance anchoring
# and a conservative blend against the warm-start free-goal proposal.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_hard_proposal_safe_seed49init_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_hard_proposal_safe_seed49init_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "TopoSSM safe hard-proposal supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type corridor_goal \
  --topo_goal_distance_weight 20.0 \
  --topo_goal_residual_scale 0.05 \
  --topo_goal_anchor_blend 0.20 \
  --topo_ssm_layers 2 \
  --topo_mamba_d_state 16 \
  --topo_mamba_d_conv 4 \
  --topo_mamba_expand 2 \
  --topo_corridor_loss_weight 0.03 \
  --topo_score_loss_weight 0.04 \
  --topo_score_temperature 0.16 \
  --distill_propose_weight 0.003 \
  --distill_refine_weight 0.015 \
  --distill_score_weight 0.04 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_ade}" ]] && ! float_le "${latest_ade}" 0.5; then
    log "Numerical/proposal meltdown latest_ade=${latest_ade} > 0.5; stopping."
    kill_run
    exit 4
  fi
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.056; then
      log "Safe hard proposal is behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.056; stopping."
      kill_run
      exit 2
    fi
  fi
