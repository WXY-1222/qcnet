...
# Safe corridor-query proposal run. Unlike the first hard corridor_query run,
# this keeps Seed49B's fallback proposal as the base and only learns a small
# corridor-conditioned residual from nearby map/corridor tokens.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_corridor_query_safe_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_corridor_query_safe_h8_f12_s5_k6_4gpu_20260505.log}"
...
log "Safe corridor-query TopoSSM decoder run started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type corridor_query_safe \
  --topo_goal_distance_weight 0.12 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.058; then
      log "Safe corridor-query behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.058; stopping."
      kill_run
      exit 2
    fi
  fi
