...
# Full-replacement TopoSSM with map lane-prior anchors on FJMP h10/f30.
...
SEED="${SEED:-23}"
RUN_TAG="${RUN_TAG:-20260508_topossm_lane_prior_fjmp_h10f30_fastval64_v1}"
...
ROUTE_LONG_SCALE="${ROUTE_LONG_SCALE:-2.0}"
ROUTE_LAT_SCALE="${ROUTE_LAT_SCALE:-1.0}"
ROUTE_TOPK="${ROUTE_TOPK:-48}"
CORRIDOR_LOSS="${CORRIDOR_LOSS:-0.00}"
SCORE_LOSS="${SCORE_LOSS:-0.0}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-60}"

RUN_NAME="topossm_lane_prior_fjmp_h10_f30_k6_4gpu_seed${SEED}_${RUN_TAG}"
...
write_summary() {
  ...
  echo "status,seed,proposal_type,lr,mamba_lr,weight_decay,route_long_scale,route_lat_scale,route_topk,corridor_loss,score_loss,max_epochs,train_bs,eval_bs,eval_batches,latest_epoch,latest_ade,best_ade,best_path" > "${SUMMARY_CSV}"
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${status}" "${SEED}" "lane_prior_anchor" "${LR}" "${MAMBA_LR}" "${WEIGHT_DECAY}" \
    "${ROUTE_LONG_SCALE}" "${ROUTE_LAT_SCALE}" "${ROUTE_TOPK}" "${CORRIDOR_LOSS}" "${SCORE_LOSS}" \
...
log "TopoSSM lane-prior FJMP h10/f30 run started"
log "repo=${REPO}"
log "data=${DATA_ROOT}/${DATA_FILE}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "summary_csv=${SUMMARY_CSV}"
log "config=seed=${SEED} proposal=lane_prior_anchor bs=${TRAIN_BS}/${EVAL_BS} eval_batches=${EVAL_BATCHES} lr=${LR} mamba_lr=${MAMBA_LR} route_scales=${ROUTE_LONG_SCALE}/${ROUTE_LAT_SCALE} topk=${ROUTE_TOPK}"
...
setsid "${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type lane_prior_anchor \
  --topo_ssm_layers 2 \
  --topo_mamba_d_state 16 \
  --topo_mamba_d_conv 4 \
  --topo_mamba_expand 2 \
  --topo_route_slot_longitudinal_scale "${ROUTE_LONG_SCALE}" \
  --topo_route_slot_lateral_scale "${ROUTE_LAT_SCALE}" \
  --topo_route_slot_topk "${ROUTE_TOPK}" \
