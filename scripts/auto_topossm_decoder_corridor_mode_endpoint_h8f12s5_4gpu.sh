...
# Structural probe for the full-replacement TopoSSM decoder.
# Starts from the best mode_endpoint checkpoint and trains a harder
# corridor-conditioned, mode-specific endpoint proposal head.
...
STOP_LATEST_ADE="${STOP_LATEST_ADE:-0.068}"
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_corridor_mode_endpoint_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_corridor_mode_endpoint_h8_f12_s5_k6_4gpu_20260505.log}"
...
log "TopoSSM corridor-mode-endpoint supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_goal_distance_weight 0.05 \
  --topo_goal_residual_scale 0.15 \
  --topo_goal_anchor_blend 0.5 \
  --topo_ssm_layers 2 \
