...
# Clean no-distill QCNet+TopoSSM refiner probe on FJMP official INTERACTION h10/f30.
# Structure: QCNet encoder + QCNet proposal/scoring decoder + TopoSSM Bi-Mamba corridor residual refiner.
...
SEED="${SEED:-23}"
RUN_TAG="${RUN_TAG:-20260508_qcnet_topossm_refiner_fjmp_h10f30_fastval64_v1}"
...
TOPO_SCORE_WEIGHT="${TOPO_SCORE_WEIGHT:-0.05}"
...
RUN_NAME="qcnet_topossm_refiner_fjmp_h10_f30_k6_4gpu_seed${SEED}_${RUN_TAG}"
...
write_summary() {
  ...
  echo "status,seed,decoder,lr,mamba_lr,weight_decay,topo_refine_weight,topo_score_weight,topo_ssm_layers,max_epochs,train_bs,eval_bs,eval_batches,latest_epoch,latest_ade,best_ade,best_path" > "${SUMMARY_CSV}"
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${status}" "${SEED}" "qcnet_topossm_refiner" "${LR}" "${MAMBA_LR}" "${WEIGHT_DECAY}" \
    "${TOPO_REFINE_WEIGHT}" "${TOPO_SCORE_WEIGHT}" "${TOPO_SSM_LAYERS}" \
...
log "QCNet+TopoSSM refiner FJMP h10/f30 run started"
log "repo=${REPO}"
log "data=${DATA_ROOT}/${DATA_FILE}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "summary_csv=${SUMMARY_CSV}"
log "config=seed=${SEED} bs=${TRAIN_BS}/${EVAL_BS} workers=${NUM_WORKERS} pin_memory=${PIN_MEMORY} eval_batches=${EVAL_BATCHES} lr=${LR} mamba_lr=${MAMBA_LR} topo_refine=${TOPO_REFINE_WEIGHT} topo_score=${TOPO_SCORE_WEIGHT} topo_layers=${TOPO_SSM_LAYERS}"
