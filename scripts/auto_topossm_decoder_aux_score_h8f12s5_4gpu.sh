...
# Isolated topology/corridor scorer.
# The Seed49B trajectory generator is loaded and frozen; only the new
# decoder.to_topo_aux_pi branch learns corridor-aware mode ranking.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_aux_score_seed49frozen_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_aux_score_seed49frozen_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "TopoSSM aux-score supervisor started"
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.056; then
      log "Aux scorer did not preserve ADE by epoch ${latest_epoch}: best_ade=${best_ade} > 0.056; stopping."
      kill_run
      exit 2
    fi
  fi
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 8 )); then
    if ! float_le "${best_ade}" 0.054; then
      log "Aux scorer still not competitive at epoch ${latest_epoch}: best_ade=${best_ade} > 0.054; stopping."
      kill_run
      exit 2
    fi
  fi
