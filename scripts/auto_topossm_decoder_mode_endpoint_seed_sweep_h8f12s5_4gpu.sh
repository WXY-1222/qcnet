...
# Clean sequential seed sweep for the best full-replacement TopoSSM decoder setup.
# This keeps the successful mode_endpoint structure fixed and varies only seed,
# so failed runs do not overwrite the current best seed49 mode_endpoint checkpoint.
...
BASELINE_ADE="${BASELINE_ADE:-0.05325273}"
STOP_BAD_ADE="${STOP_BAD_ADE:-0.058}"
...
run_seed() {
  local seed="$1"
  local save_root="${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_seed${seed}_h8_f12_s5_k6_4gpu_20260505"
  local run_log="${LOG_DIR}/qcnet_topossm_decoder_mode_endpoint_seed${seed}_h8_f12_s5_k6_4gpu_20260505.log"
  local ckpt_dir="${save_root}/lightning_logs/version_0/checkpoints"

  rm -rf "${save_root}/lightning_logs/version_0"
  mkdir -p "${save_root}"

  log "===== START mode_endpoint seed=${seed} ====="
  ...
  while kill -0 "${pid}" 2>/dev/null; do
    ...
    log "seed=${seed} latest_epoch=${latest_epoch:-na} latest_ade=${latest_ade:-na} best_ade=${best_ade:-na} best_path=${best_path:-na}"
    update_best_overall "${seed}" "${best_ade}" "${best_path}"

    if [[ -n "${best_ade}" ]] && float_le "${best_ade}" "${TARGET_ADE}"; then
      log "seed=${seed} reached target best_ade=${best_ade} <= ${TARGET_ADE}; stopping sweep."
      stop_train "${save_root}"
      return 0
    fi
    if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 3 )); then
      if ! float_le "${best_ade}" "${STOP_BAD_ADE}"; then
        log "seed=${seed} behind after epoch ${latest_epoch}: best_ade=${best_ade} > ${STOP_BAD_ADE}; switching seed."
        stop_train "${save_root}"
        return 2
      fi
    fi
    if [[ -n "${latest_epoch}" && -n "${latest_ade}" ]] && (( latest_epoch >= 1 )); then
      if ! float_le "${latest_ade}" "${STOP_LATEST_ADE}"; then
        log "seed=${seed} unstable latest_ade=${latest_ade} > ${STOP_LATEST_ADE}; switching seed."
        stop_train "${save_root}"
        return 2
      fi
    fi
  ...
  best_path="$(json_value "${metrics}" best_path)"
  update_best_overall "${seed}" "${best_ade}" "${best_path}"
  log "seed=${seed} finished. metrics=${metrics}"
  if [[ -n "${best_ade}" ]] && float_le "${best_ade}" "${TARGET_ADE}"; then
    return 0
  fi
  return 2
...
log "TopoSSM mode_endpoint seed sweep started"
log "Repo=${REPO} GPUs=${GPU_IDS} seeds=${SEEDS} target=${TARGET_ADE} baseline=${BASELINE_ADE}"

for seed in ${SEEDS}; do
  status=0
  run_seed "${seed}" || status=$?
  if [[ "${status}" == "0" ]]; then
    log "Seed sweep reached target. best_seed=${BEST_OVERALL_SEED} best_ade=${BEST_OVERALL_ADE} best_path=${BEST_OVERALL_PATH}"
    exit 0
  fi
done

log "Seed sweep finished without reaching target ${TARGET_ADE}."
...
if [[ -n "${BEST_OVERALL_ADE}" ]] && float_le "${BEST_OVERALL_ADE}" "${BASELINE_ADE}"; then
  log "Seed sweep found a checkpoint that beats baseline ${BASELINE_ADE}."
  exit 0
fi
exit 1
