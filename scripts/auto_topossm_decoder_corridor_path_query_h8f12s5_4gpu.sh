...
# Structural proposal-coverage run:
# replace the free goal MLP proposal with corridor/path-conditioned queries.
# Each TopoSSM mode picks a high-scoring map corridor candidate and builds its
# whole initial anchor by snapping the interpolated path to nearby map nodes.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_corridor_path_query_h8_f12_s5_k6_4gpu_20260505}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_corridor_path_query_h8_f12_s5_k6_4gpu_20260505.log}"
CKPT_DIR="${SAVE_ROOT}/lightning_logs/version_0/checkpoints"
DIAG_ROOT="${DIAG_ROOT:-${BASE_OUT}/diagnostics/topossm_corridor_path_query_20260505}"
...
log "Corridor-path-query TopoSSM decoder supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type corridor_path_query \
  --topo_goal_distance_weight 0.06 \
  --topo_goal_residual_scale 0.30 \
  --topo_goal_anchor_blend 0.85 \
  --topo_ssm_layers 2 \
  --topo_mamba_d_state 16 \
  --topo_mamba_d_conv 4 \
  --topo_mamba_expand 2 \
  --topo_corridor_loss_weight 0.004 \
  --topo_score_loss_weight 0.004 \
  --topo_score_temperature 0.20 \
  --distill_propose_weight 0.02 \
...
while is_pid_running "${pid}"; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
    if ! float_le "${best_ade}" 0.058; then
      log "Corridor-path-query behind at epoch ${latest_epoch}: best_ade=${best_ade} > 0.058; stopping."
      kill_run
      run_diag "${best_path}"
      exit 2
    fi
