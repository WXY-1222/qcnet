...
# PaperA-v6 conservative warm-start:
# keep mode_endpoint goal head intact and only replace the straight coarse path
# with a tiny 1-control-point polyline anchor head.
...
SEED="${SEED:-17}"
RUN_TAG="${RUN_TAG:-20260506_paperA_v6_lite_warmstart_seed17}"
...
STOP_IF_BEST_GT_E3="${STOP_IF_BEST_GT_E3:-0.0630}"

SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_v6_lite_warmstart_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_v6_lite_warmstart_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA-v6 lite warm-start started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "init_ckpt=${INIT_CKPT}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=proposal=mode_endpoint_polyline_lite endpoint_scale=${TOPO_MODE_ENDPOINT_SCALE} polyline_scale=${TOPO_POLYLINE_CONTROL_SCALE} lr=${LR}"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type mode_endpoint_polyline_lite \
  --topo_ssm_layers 2 \
