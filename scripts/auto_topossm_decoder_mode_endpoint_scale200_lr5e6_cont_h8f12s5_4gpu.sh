...
# Ultra-low-LR continuation from the independently validated scale=0.20
# TopoSSM mode-endpoint checkpoint. This is a short probe to test whether the
# new scale upper-bound checkpoint can be nudged below its current best without
# drifting away from the sharp epoch-2 optimum.
...
SEED="${SEED:-42}"
BASELINE_ADE="${BASELINE_ADE:-0.04331298}"
TARGET_ADE="${TARGET_ADE:-0.0425}"
STOP_LATEST_ADE="${STOP_LATEST_ADE:-0.0475}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-120}"
RUN_TAG="${RUN_TAG:-20260505_scale200_lr5e6_cont}"

INIT_CKPT="${INIT_CKPT:-${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_scale200_seed42_h8_f12_s5_k6_4gpu_20260505_scale_up2_bound/lightning_logs/version_0/checkpoints/epoch=2-step=645.ckpt}"
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_decoder_mode_endpoint_scale200_lr5e6_cont_seed42_h8_f12_s5_k6_4gpu_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_decoder_mode_endpoint_scale200_lr5e6_cont_seed42_h8_f12_s5_k6_4gpu_${RUN_TAG}.log}"
...
log "TopoSSM mode-endpoint scale=0.20 lr=5e-6 continuation started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED} baseline=${BASELINE_ADE} target=${TARGET_ADE}"
log "Init=${INIT_CKPT}"
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 1 )); then
    if ! float_lt "${best_ade}" "${BASELINE_ADE}"; then
      log "No improvement after epoch ${latest_epoch}: best_ade=${best_ade} >= baseline ${BASELINE_ADE}; stopping run."
      stop_train
      exit 2
    fi
  fi
...
if [[ -n "${best_ade}" ]] && float_lt "${best_ade}" "${BASELINE_ADE}"; then
  log "Continuation improved baseline: best_ade=${best_ade} < ${BASELINE_ADE}"
  exit 0
fi
exit 1
