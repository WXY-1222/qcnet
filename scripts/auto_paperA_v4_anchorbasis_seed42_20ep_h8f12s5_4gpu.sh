...
# PaperA-v4:
# keep mode_endpoint proposal intact, but replace the straight goal anchor
# with a goal-aligned mode-specific anchor basis before TopoSSM refinement.
...
SEED="${SEED:-42}"
RUN_TAG="${RUN_TAG:-20260506_paperA_v4_anchorbasis_seed42_20ep}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-180}"

TOPO_MODE_ENDPOINT_SCALE="${TOPO_MODE_ENDPOINT_SCALE:-0.20}"
TOPO_ANCHOR_BASIS_SCALE="${TOPO_ANCHOR_BASIS_SCALE:-0.10}"
...
TARGET_ADE="${TARGET_ADE:-0.0430}"
STOP_IF_BEST_GT_E2="${STOP_IF_BEST_GT_E2:-0.0550}"
STOP_IF_BEST_GT_E4="${STOP_IF_BEST_GT_E4:-0.0485}"

SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_v4_anchorbasis_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_v4_anchorbasis_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA-v4 anchor-basis fresh probe started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=from_scratch decoder=topossm proposal=mode_endpoint_anchorbasis endpoint_scale=${TOPO_MODE_ENDPOINT_SCALE} anchor_basis_scale=${TOPO_ANCHOR_BASIS_SCALE} epochs=${MAX_EPOCHS} lr=${LR}"
