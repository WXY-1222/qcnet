...
# Sequential scale sweep for the best full-replacement TopoSSM mode-endpoint setup.
# It restarts from the same Seed49B base instead of continuing the seed42 best,
# so the current best checkpoint is preserved and each scale gets a clean run.
...
SCALES="${SCALES:-0.06 0.08 0.10 0.12}"
SEED="${SEED:-42}"
TARGET_ADE="${TARGET_ADE:-0.0505}"
BASELINE_ADE="${BASELINE_ADE:-0.05115576}"
STOP_BAD_ADE="${STOP_BAD_ADE:-0.0565}"
STOP_LATEST_ADE="${STOP_LATEST_ADE:-0.075}"
MONITOR_INTERVAL_SEC="${MONITOR_INTERVAL_SEC:-180}"
RUN_TAG="${RUN_TAG:-20260505}"
...
run_scale() {
  ...
  log "===== START mode_endpoint scale=${scale} seed=${SEED} ====="
  ...
  while kill -0 "${pid}" 2>/dev/null; do
    ...
    if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 4 )); then
      if ! float_le "${best_ade}" "${BASELINE_ADE}"; then
        log "scale=${scale} has not beaten seed42 baseline after epoch ${latest_epoch}: best_ade=${best_ade} > ${BASELINE_ADE}; switching scale."
        stop_train "${save_root}"
        return 2
      fi
    fi
    if [[ -n "${latest_epoch}" && -n "${best_ade}" ]] && (( latest_epoch >= 3 )); then
      if ! float_le "${best_ade}" "${STOP_BAD_ADE}"; then
        log "scale=${scale} behind after epoch ${latest_epoch}: best_ade=${best_ade} > ${STOP_BAD_ADE}; switching scale."
        stop_train "${save_root}"
        return 2
      fi
    fi
...
log "TopoSSM mode_endpoint scale sweep started"
...
for scale in ${SCALES}; do
  ...
  if [[ "${status}" == "0" ]]; then
    log "Scale sweep reached target. best_scale=${BEST_OVERALL_SCALE} best_ade=${BEST_OVERALL_ADE} best_path=${BEST_OVERALL_PATH}"
    exit 0
  fi
done

log "Scale sweep finished without reaching target ${TARGET_ADE}."
...
if [[ -n "${BEST_OVERALL_ADE}" ]] && float_le "${BEST_OVERALL_ADE}" "${BASELINE_ADE}"; then
  log "Scale sweep found a checkpoint that beats seed42 baseline ${BASELINE_ADE}."
  exit 0
fi
exit 1
