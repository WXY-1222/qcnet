...
# start from the best non-distill PaperA fresh checkpoint, swap only the
# coarse anchor generator to mode_endpoint_anchorbasis, and continue training
# with the same task/loss (no teacher, no distillation).
...
SEED="${SEED:-17}"
RUN_TAG="${RUN_TAG:-20260506_paperA_v4_anchorbasis_from_clean_seed17}"
...
TOPO_ANCHOR_BASIS_SCALE="${TOPO_ANCHOR_BASIS_SCALE:-0.06}"
...
TARGET_ADE="${TARGET_ADE:-0.0580}"
STOP_IF_BEST_GT_E2="${STOP_IF_BEST_GT_E2:-0.0645}"
STOP_IF_BEST_GT_E4="${STOP_IF_BEST_GT_E4:-0.0620}"

SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/paperA_v4_anchorbasis_from_clean_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/paperA_v4_anchorbasis_from_clean_h8_f12_s5_k6_4gpu_seed${SEED}_${RUN_TAG}.log}"
...
log "PaperA-v4 anchor-basis clean continuation started"
log "Repo=${REPO} GPUs=${GPU_IDS} seed=${SEED}"
log "init_ckpt=${INIT_CKPT}"
log "save_root=${SAVE_ROOT}"
log "run_log=${RUN_LOG}"
log "config=init_from_clean proposal=mode_endpoint_anchorbasis endpoint_scale=${TOPO_MODE_ENDPOINT_SCALE} anchor_basis_scale=${TOPO_ANCHOR_BASIS_SCALE} epochs=${MAX_EPOCHS} lr=${LR}"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_proposal_type mode_endpoint_anchorbasis \
  --topo_ssm_layers 2 \
...
while kill -0 "${pid}" 2>/dev/null; do
  ...
  if [[ -n "${best_ade}" && -n "${latest_epoch}" ]]; then
    ...
    if [[ "${latest_epoch}" -ge 4 ]] && float_gt "${best_ade}" "${STOP_IF_BEST_GT_E4}"; then
