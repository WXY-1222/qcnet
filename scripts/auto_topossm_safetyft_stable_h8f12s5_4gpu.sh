...
# Stable SafetyFT continuation.
# This keeps the strongest observed QCNet decoder + TopoSSM refiner structure
# and only fine-tunes it conservatively with smaller topology losses.
...
SAVE_ROOT="${SAVE_ROOT:-${BASE_OUT}/qcnet_topossm_safetyft_stable_h8_f12_s5_k6_4gpu_20260504}"
RUN_LOG="${RUN_LOG:-${LOG_DIR}/qcnet_topossm_safetyft_stable_h8_f12_s5_k6_4gpu_20260504.log}"
...
log "SafetyFT-stable supervisor started"
...
"${PYTHON_BIN}" train_qcnet.py \
  ...
  --topo_refine_weight 0.08 \
  --topo_score_weight 0.08 \
  --topo_ssm_layers 1 \
